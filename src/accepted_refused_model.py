...
from sklearn.metrics import roc_auc_score, roc_curve, auc, confusion_matrix, f1_score, recall_score, accuracy_score
import json
...
def outputGridSearchLogisticRegression(path=DATA + 'accepted_refused_ds.csv'):
    ...
    y_0 = [p[1] for p in y_pred]
    fpr, tpr, threshold = roc_curve(y_true=y_test, y_score=y_0)
    l = np.arange(len(tpr))
    roc = pd.DataFrame(
    ...
    print confusion_matrix(y_true=y_test, y_pred=[1 if a > 0.5 else 0 for a in y_0])


    print 'Score f1 (thres = ' + str(thre) + ') = ' + str(f1_score(y_true=y_test, y_pred=[1 if a > thre else 0 for a in y_0]))

    print 'Score f1 (thres = 0.5) = ' + str(f1_score(y_true=y_test, y_pred=[1 if a > 0.5 else 0 for a in y_0]))

    print 'Score recall (thres = ' + str(thre) + ') = ' + str(recall_score(y_true=y_test, y_pred=[1 if a > thre else 0 for a in y_0]))

    print 'Score recall (thres = 0.5) = ' + str(recall_score(y_true=y_test, y_pred=[1 if a > 0.5 else 0 for a in y_0]))

    print 'Score accuracy (thres = ' + str(thre) + ') = ' + str(accuracy_score(y_true=y_test, y_pred=[1 if a > thre else 0 for a in y_0]))
